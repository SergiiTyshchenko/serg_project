...
import com.epam.training.cockpits.constants.TrainCockpitsConstants;
 ...
 */
@SystemSetup(extension = TrainCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(TrainCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(TrainCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						TrainCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/traincockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
